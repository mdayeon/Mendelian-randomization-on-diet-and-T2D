### Rscript for Single-mediator multivariable MR (MVMR) using IVW on associations identified from MR-RAPS.
...
#Run this loop for multivariable MR analysis.
for (e in exposure){
  for (t in outcome){
    for (m in mediator){
      #Load the data for exposure and mediator. 
      ...
      write.table(result_mvmr, file=paste0(output_dir,"Multivariable_MR_",e,"_",m,"_",t,"_mvmr_ivw_results.txt"), sep='\t',row.names=F, quote=F)
    }
  }
}
